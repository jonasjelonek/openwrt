		  ...
		  size_t *erofs_len)
{
	...
	err = mtd_read(master, offset + EROFS_SB_OFFSET, sizeof(sb), &retlen,
		       (void *)&sb);
	if (err || (retlen != sizeof(sb))) {
		pr_alert("error occured while reading from \"%s\"\n",
			 master->name);
	...
	retlen = blksize * blocks;
	if (offset + EROFS_SB_OFFSET + retlen > master->size) {
			   ...
			   enum mtdsplit_part_type *type)
{
	...
	ret = mtd_read(mtd, offset + EROFS_SB_OFFSET, sizeof(magic),
