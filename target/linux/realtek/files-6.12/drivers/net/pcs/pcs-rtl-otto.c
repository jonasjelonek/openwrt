					 ...
					 enum rtpcs_port_media port_media)
{
	...
	rtpcs_sds_write_bits(sds, 0x2a, 0x7, 15, 15, is_10g ? 0x1 : 0x0);
	...
	case RTPCS_PORT_MEDIA_FIBER_10G:
		// TODO: where to get that value from??
				   ...
				   phy_interface_t mode)
{
	...
	switch (hw_mode) {
	case RTPCS_SDS_MODE_OFF:
		port_media = RTPCS_PORT_MEDIA_NONE;
		break;
	case RTPCS_SDS_MODE_SGMII:
	case RTPCS_SDS_MODE_1000BASEX:
		port_media = RTPCS_PORT_MEDIA_FIBER_1G;
		break;
	default:
		port_media = RTPCS_PORT_MEDIA_FIBER_10G;
		break;
	}

	ret = rtpcs_931x_sds_set_port_media(sds, port_media);
	if (ret < 0)
		return ret;
